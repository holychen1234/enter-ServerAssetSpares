...
export default function NetworkDeviceList() {
  ...
  const [toDelete, setToDelete] = useState<NetworkDevice | null>(null);
  ...
  const idcOptions = useMemo(() => {
    return Array.from(new Set(devices.map((d) => d.idc))).sort();
  }, [devices]);
  ...
  const filtered = useMemo(() => {
    ...
    return devices.filter((d) => {
      ...
      if (idcFilter !== "all" && d.idc !== idcFilter) return false;
      if (!kw) return true;
      return (
        d.hostname.toLowerCase().includes(kw) ||
        d.sn.toLowerCase().includes(kw) ||
        d.assetTag.toLowerCase().includes(kw) ||
        d.mgmtIp.includes(kw) ||
        d.model.toLowerCase().includes(kw) ||
        d.tags.join(",").toLowerCase().includes(kw)
      );
    });
  }, [devices, search, statusFilter, typeFilter, idcFilter]);
  ...
  const mCreate = useMutation({
    mutationFn: createNetworkDevice,
    onSuccess: () => {
  ...
  const mUpdate = useMutation({
    mutationFn: (vars: { id: string; patch: Partial<NetworkDevice> }) =>
      updateNetworkDevice(vars.id, vars.patch),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["network-devices"] });
      toast({ title: "网络设备已更新" });
      setFormOpen(false);
  ...
  const mDelete = useMutation({
    mutationFn: deleteNetworkDevice,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["network-devices"] });
      toast({ title: "网络设备已删除" });
      setToDelete(null);
  ...
  const mBatchDelete = useMutation({
    mutationFn: batchDeleteNetworkDevices,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["network-devices"] });
      toast({ title: `已删除 ${selectedIds.size} 台设备` });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="网络设备"
        description="管理交换机、路由器、防火墙、负载均衡等网络设备"
            ...
            {canEdit && (
              <>
                ...
                <Button onClick={() => { setEditing(null); setFormOpen(true); }}>
                  <Plus className="mr-1 h-4 w-4" /> 新增设备
                </Button>
              </>
            )}
          </div>
      ...
      <Card className="shadow-card-soft">
        <div className="border-b border-border p-4">
          <DataTableToolbar
            ...
            placeholder="搜索 设备名 / SN / 资产编号 / 管理 IP / 型号"
            filters={
              <>
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-3 border-b border-border bg-muted/40 px-4 py-2">
            <span className="text-sm text-muted-foreground">
              已选 <span className="font-medium text-foreground">{selectedIds.size}</span> 台设备
            </span>
            <Button
              variant="ghost"
              size="sm"
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort("deviceType")}>
                  <span className="inline-flex items-center">
                    类型 <SortIcon field="deviceType" />
                  </span>
                </TableHead>
                <TableHead>厂商 / 型号</TableHead>
                <TableHead>位置 (IDC)</TableHead>
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      <span className="font-mono text-xs text-muted-foreground">{d.sn} · {d.assetTag}</span>
                    </div>
                  </TableCell>
                  <TableCell><StatusBadge kind="server" value={d.status} /></TableCell>
                  <TableCell>
                    <span className="rounded bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                      {deviceTypeLabel[d.deviceType] || d.deviceType}
                    </span>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm">{d.manufacturer}</span>
                  ...
                  <TableCell>
                    <div className="flex flex-col text-sm">
                      <span>{d.idc}</span>
                      <span className="text-xs text-muted-foreground">{d.rack} · {d.uPosition}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className="font-mono text-sm">{d.mgmtIp}</span>
                  </TableCell>
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>删除网络设备</AlertDialogTitle>
            <AlertDialogDescription>
      ...
      <AlertDialog open={batchDeleteOpen} onOpenChange={(v) => !v && setBatchDeleteOpen(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>批量删除网络设备</AlertDialogTitle>
            <AlertDialogDescription>
              确认删除已选的{" "}
