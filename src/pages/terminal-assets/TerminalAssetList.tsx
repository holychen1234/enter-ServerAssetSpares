...
import { TerminalAssetForm } from "./TerminalAssetForm";
...
import { exportTerminalAssetsToCsv, exportTerminalAssetsToExcel, downloadBlob } from "@/lib/import-export";
...
export default function TerminalAssetList() {
  ...
  const { data: assets = [], isLoading } = useQuery({
    queryKey: ["terminal-assets"],
    queryFn: listTerminalAssets,
  });
  ...
  const [editing, setEditing] = useState<TerminalAsset | null>(null);
  const [formOpen, setFormOpen] = useState(false);
  const [toDelete, setToDelete] = useState<TerminalAsset | null>(null);
  ...
  const osOptions = useMemo(() => {
    return Array.from(new Set(assets.map((a) => a.os))).sort();
  }, [assets]);
  ...
  const filtered = useMemo(() => {
    ...
    return assets.filter((a) => {
      ...
      if (!kw) return true;
      return (
        a.hostname.toLowerCase().includes(kw) ||
        a.sn.toLowerCase().includes(kw) ||
        a.assetTag.toLowerCase().includes(kw) ||
  ...
  }, [assets, search, statusFilter, osFilter]);
  ...
  const mCreate = useMutation({
    mutationFn: createTerminalAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["terminal-assets"] });
      toast({ title: "终端资产已新增" });
      setFormOpen(false);
    },
  });
  const mUpdate = useMutation({
    mutationFn: (vars: { id: string; patch: Partial<TerminalAsset> }) =>
      updateTerminalAsset(vars.id, vars.patch),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["terminal-assets"] });
      toast({ title: "终端资产已更新" });
      setFormOpen(false);
  ...
  const mDelete = useMutation({
    mutationFn: deleteTerminalAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["terminal-assets"] });
      toast({ title: "终端资产已删除" });
      setToDelete(null);
  ...
  const mBatchDelete = useMutation({
    mutationFn: batchDeleteTerminalAssets,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["terminal-assets"] });
      toast({ title: `已删除 ${selectedIds.size} 台终端资产` });
  ...
  return (
    <div className="space-y-6">
            ...
            {canEdit && (
              <>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => {
                      const blob = exportTerminalAssetsToCsv(filtered);
                      downloadBlob(blob, `终端资产_${new Date().toISOString().slice(0, 10)}.csv`);
                    }}>
                      导出 CSV
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => {
                      const blob = exportTerminalAssetsToExcel(filtered);
                      downloadBlob(blob, `终端资产_${new Date().toISOString().slice(0, 10)}.xlsx`);
                    }}>
                      导出 Excel
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                ...
                <Button onClick={() => { setEditing(null); setFormOpen(true); }}>
                  <Plus className="mr-1 h-4 w-4" /> 新增终端
                </Button>
              </>
            )}
          </div>
            ...
            <TableBody>
              ...
              {paged.map((a) => (
                <TableRow
                  ...
                  onClick={() => navigate(`/terminal-assets/${a.id}`)}
                >
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      <span className="font-mono text-xs text-muted-foreground">{a.sn} · {a.assetTag}</span>
                    </div>
                  </TableCell>
                  <TableCell><StatusBadge kind="server" value={a.status} /></TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm">{a.manufacturer}</span>
                      <span className="text-xs text-muted-foreground">{a.model}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm">{a.os}</span>
                      {a.osVersion && (
                        <span className="text-xs text-muted-foreground">{a.osVersion}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {a.bizIp ? (
                      <span className="font-mono text-sm">{a.bizIp}</span>
                    ) : (
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>删除终端资产</AlertDialogTitle>
            <AlertDialogDescription>
      ...
      <AlertDialog open={batchDeleteOpen} onOpenChange={(v) => !v && setBatchDeleteOpen(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>批量删除终端资产</AlertDialogTitle>
            <AlertDialogDescription>
              确认删除已选的{" "}
      ...
      <ImportDialog
        ...
        onImported={() => queryClient.invalidateQueries({ queryKey: ["terminal-assets"] })}
        entityType="terminal-asset"
      />
    </div>
  );
}
